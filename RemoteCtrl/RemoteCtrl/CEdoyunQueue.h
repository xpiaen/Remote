...
private:
	...
	void threadMain() {
		...
		while (GetQueuedCompletionStatus(m_hCompeletionPort, &dwTraansferred, &CompletionKey, &pOverlapped, INFINITE)) {
		...
		while (GetQueuedCompletionStatus(m_hCompeletionPort, &dwTraansferred, &CompletionKey, &pOverlapped, 0)) {
			if ((dwTraansferred == 0) || (CompletionKey == NULL)) {
				printf("thread is prepare to exit!\r\n");
				continue;
			}
			pParam = (PPARAM*)CompletionKey;
